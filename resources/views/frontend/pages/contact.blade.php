        ...
        <div class="mb-4 grid md:grid-cols-2 overflow-auto md:px-20 gap-8 items-center">
            <div class="text-left">
                <div class="flex items-center relative">
                    <div class="rounded-full h-3.5 w-3.5 bg-amber-400 animate-ping absolute opacity-75 overflow-hidden ping">
                    </div>
                    <h4 class="relative left-6 text-2xl text-gray-800 text-left font-bold my-2">{{ __("We're located at") }}
                    </h4>
                </div>
                <h4 class="text-lg text-gray-700 text-left font-semibold">{{ __($info->location) }}</h4>

                <p class="text-gray-700 font-semibold">{{ __('Pulhat - Uttar Faridpur Gorstan Rd, Dinajpur, Bangladesh') }}
                </p>
                <p><span class="font-semibold text-gray-700">{{ __('Phone') }}:</span>: <a href="tel:{{ __($info->phone) }}"
                        class="text-gray-600">{{ __($info->phone) }}</a>
                    , <a href="tel:{{ __($info->phone_2) }}" class="text-gray-600">{{ __($info->phone_2) }}</a>
                </p>
                <p><span class="font-semibold text-gray-700">{{ __('Email') }}:</span> <a
                        href="mailto:{{ __($info->email) }}" class="text-gray-600">
                        {{ __($info->email) }}</a></p>
            ...
            <div class="rounded overflow-hidden h-[200px] w-full object-cover">
                @if (Illuminate\Support\Facades\File::exists('images/institute/' . $info->institute_image))
                    <img src="'{{ 'data:image/png;base64,' . base64_encode(file_get_contents(public_path('images/institute/' . $info->institute_image))) }}"
                        alt="dti">
                @endif
